...
// ---------- Domain loading (dipakai bersama semua tab) ----------
...
async function loadDomains() {
    ...
    try {
        ...
        await Promise.all([loadSubcat(), loadCSATree()]);
    } catch (err) {
...
domainSel.addEventListener("change", () => { loadSubcat(); loadCSATree(); });

// =====================================================================
// TAB: Statistik menurut Subjek (CSA Subject browsing + fast download)
// =====================================================================
...
async function selectSubject(subId, title, el) {
    ...
    try {
        ...
        currentRows = items.map(it => ({
            id: it.id, title: it.title, tablesource: it.tablesource,
